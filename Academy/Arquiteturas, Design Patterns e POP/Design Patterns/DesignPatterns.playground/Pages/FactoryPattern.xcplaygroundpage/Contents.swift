...
let computer = ComputerFactory.makeComputer(system: OperatingSystem(version: 14, type: .iOS))
print(computer.aboutDevice())

var system: [OperatingSystem] = [
    OperatingSystem(version: 14, type: .iOS),
    OperatingSystem(version: 11, type: .macOS),
    OperatingSystem(version: 15, type: .iPadOS)
]

system.forEach{ print(ComputerFactory.makeComputer(system: $0).aboutDevice())}
